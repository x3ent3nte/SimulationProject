...
VkDescriptorSetLayout TimeAdvancerUtil::createDescriptorSetLayout(VkDevice logicalDevice) {
    return Compute::createDescriptorSetLayout(logicalDevice, kNumberOfBindings);
}

VkDescriptorPool TimeAdvancerUtil::createDescriptorPool(VkDevice logicalDevice, size_t maxSets) {
    return Compute::createDescriptorPool(logicalDevice, kNumberOfBindings, maxSets);
}

VkDescriptorSet TimeAdvancerUtil::createDescriptorSet(
...
VkCommandBuffer TimeAdvancerUtil::createCommandBuffer(
    ...
    uint32_t xGroups = ceil(((float) numberOfElements) / ((float) TimeAdvancerUtil::xDim));
